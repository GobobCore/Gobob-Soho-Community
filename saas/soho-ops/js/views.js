...
(function () {
  ...
  const OrgDetailView = defineComponent({
    components: { Card, Stat },
    props: ["ctx"],
    data: () => ({ org: null, invoices: [], usage: [], loading: true, editing: false, form: {}, newInv: null, showNewInv: false, copied: false }),
    template: `
    <div>
      ...
      <template v-else-if="org">
        <div class="flex items-start justify-between mb-6">
          <div>
            ...
            <!-- R-Feat 2026-09-16: SaaS 多机构获客链接 -->
            <div v-if="org.slug" class="mt-2 text-xs">
              <span class="text-slate-400">获客链接:</span>
              <code class="ml-1 bg-slate-100 px-2 py-0.5 rounded text-primary-700 font-mono">
                https://portal.gobob.cn/?org={{ org.slug }}
              </code>
              <button @click="copyLink" class="ml-2 text-primary-600 hover:underline text-xs">
                {{ copied ? '已复制' : '复制' }}
              </button>
            </div>
          </div>
    ...
    methods: {
      ...
      copyLink() {
        const url = `https://portal.gobob.cn/?org=${this.org.slug}`;
