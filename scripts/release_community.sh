...
# 2. 检查 saas/ 在 git index 里 (确保不会 commit)
echo ""
echo "=== 2. 检查 saas/ 不在 git index 里 ==="
if git ls-files saas/ | grep -q .; then
    echo "  ❌ saas/ 文件在 git index 里, 必须先 git rm saas/ 才能继续"
    exit 1
fi
echo "  ✓ saas/ 不在 git index 里"
...
if [[ -n "$DRY_RUN" ]]; then
    ...
    git fetch "$COMMUNITY_REPO" "$BRANCH" 2>&1 | tail -1
    SAAS_COUNT=$(git ls-tree -r "$COMMUNITY_REPO/$BRANCH" 2>/dev/null | grep -c "/saas/" || true)
    if [[ "$SAAS_COUNT" -gt 0 ]]; then
